...
public class PlaceManager : MonoBehaviour
{
    ...
    public void SetAllConnectedPlaces()
    {
        foreach (var place1 in allPlaceGos)
        {
            ...
            DrawLine(place1.place);
        }
    }
    void DrawLine(Place place)
    {
        foreach (var item in place.connectionBetweenPlaces)
        {
            var line = Instantiate(lineRenderer);
            //line.positionCount = (item.inBetweenPlaces.Count * 2) - 1;
            line.SetColors(Random.ColorHSV(), Random.ColorHSV());


            line.gameObject.name = item.inBetweenPlaces[0].name;
            List<Vector3> positions = new();
            positions.Add(item.inBetweenPlaces[0].point.position + Vector3.up);

            for (int i = 1, ii = 1; i < item.inBetweenPlaces.Count; i++)
            {
                line.gameObject.name += "=> " + item.inBetweenPlaces[i].name;
                line.SetPosition(ii, GetCommonPath(item.inBetweenPlaces[i - 1], item.inBetweenPlaces[i]).position + Vector3.up);
                //ii++;
                positions.Add(GetCommonPath(item.inBetweenPlaces[i - 1], item.inBetweenPlaces[i]).position + Vector3.up);
                //line.SetPosition(ii, item.inBetweenPlaces[i].point.position + Vector3.up);
                positions.Add(item.inBetweenPlaces[i].point.position + Vector3.up);
